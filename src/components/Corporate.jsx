          ...
          <div className="mt-8 space-y-6">
            {corporate.offerings.map((o) => (
              <div key={o.title}>
                ...
                <p className="text-sand-2/90 text-lg leading-relaxed">{o.text}</p>
              </div>
            ))}
          </div>
        </Reveal>
        <Reveal delay={0.1}>
          <p className="font-mono text-[11px] uppercase tracking-widest text-sand-2/85 mb-5 mt-2 lg:mt-0">
            Why choose MALDI for corporate
          </p>
          <ul className="grid sm:grid-cols-2 gap-x-6 gap-y-3">
            {corporate.reasons.map((r) => (
              <li key={r} className="flex gap-2.5 text-lg text-sand-2/85">
